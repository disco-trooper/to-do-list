            ...
            for (let property in project) {
              if (project[property].title == todoName) {
                ...
                for (property in project) {
                  if (property != 'title') {
                    delete Object.assign(project, {
                      [counter]: project[property],
                    })[property];
                    counter++;
                  }
                }
          ...
          if (project.title == getSelectedProjectName()) {
            ...
            if (newDate) {
              if (/^[0-9]{4}\-[0-9]{1,2}\-[0-9]{1,2}$/.test(newDate) != true) {
                document
                  .querySelector('#messageDate')
                  .classList.add('is-active');
                return;
              }
            }
            project[Object.keys(project).length - 1] = factories.todoFactory(
              newTitle,
              newDesc,
              newDate,
              newPriority
    ...
    document.addEventListener('click', (event) => {
      ...
      if (event.target.classList.contains('editTodo')) {
        ...
        document.querySelector('#editTodoDesc').value =
          event.target.parentElement.parentElement.lastChild.textContent;
        let formatedDate = event.target.parentElement.parentElement.firstElementChild.nextSibling.textContent.slice(
          5
        );
        document.querySelector('#editTodoDate').value = format(
          parse(formatedDate, 'do MMM yyyy', new Date()),
          'yyyy-M-d'
        );
      }
          ...
          if (project.title == getSelectedProjectName()) {
            for (let toDo in project) {
              if (project[toDo].title == selectedTodoName) {
                ...
                if (newDate) {
                  if (
                    /^[0-9]{4}\-[0-9]{1,2}\-[0-9]{1,2}$/.test(newDate) != true
                  ) {
                    document
                      .querySelector('#messageDate')
                      .classList.add('is-active');
