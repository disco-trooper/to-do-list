...
const todoViewer = (() => {
  function editTodoModalHandler(event) {
    ...
    if (
      event.target.getAttribute('id') == 'editTodoModalBackground' ||
      event.target.getAttribute('id') == 'editTodoModalClose' ||
  ...
  function addTodoModalHandler(event) {
    // Close Edit To-Do Modal
    if (
      event.target.getAttribute('id') == 'addTodoModalBackground' ||
      event.target.getAttribute('id') == 'addTodoModalClose' ||
      event.target.classList.contains('cancel')
    ) {
  ...
  function messageModalHandler(event) {
    // Close date message
    if (
      event.target.getAttribute('id') == 'messageBackgroundDate' ||
      event.target.getAttribute('id') == 'messageCloseDate'
    ) {
    ...
    // Close title message
    if (
      event.target.getAttribute('id') == 'messageBackgroundTitle' ||
      event.target.getAttribute('id') == 'messageCloseTitle'
    ) {
  ...
  function showTodos(eventName, projects) {
    ...
    projects.forEach((project) => {
      if (project.title == eventName) {
        let currentColumn = 0;
        const toDos = Object.keys(project);
        toDos.forEach((toDo) => {
